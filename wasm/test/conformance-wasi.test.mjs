...
//
// Checks 10 and 11 (the callback hooks) are not covered — same reason as the
// Emscripten suite: exposing them costs bundle size for a feature a DOM
// sanitizer rarely wants. Omitted deliberately, not forgotten.

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { readFile } from 'node:fs/promises';
import { HtmlSanitizer } from '../dist/htmlsanitizer-wasi.mjs';

// Node has no global fetch for file:// URLs, so hand the bytes in directly.
// A browser would use the zero-arg form and fetch the .wasm alongside the .mjs.
const wasmBytes = await readFile(new URL('../dist/htmlsanitizer-wasi.wasm', import.meta.url));
const s = await HtmlSanitizer.create({ wasmBytes });
...
function await_fresh() {
  return new HtmlSanitizer(s._inst);
}
